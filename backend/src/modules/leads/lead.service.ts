...
import { LEAD_TRANSITIONS, type CreateLeadInput, type LeadRepository, type LeadStatus } from './lead.types.js';
...
export class LeadService {
  ...
  async qualify(organizationId: string, id: string, score: number, qualified: boolean, reason: string) {
  ...
  async convert(organizationId: string, id: string, customerId: string) {
    const lead = await this.repository.findById(organizationId, id);
    if (!lead) throw notFound('Lead');
